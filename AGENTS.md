...
- **Enum name maps**: Each enum gets `TypeName_name` (int32→string) and `TypeName_value` (string→int32) maps, plus a `String()` method. Enum constants are prefixed with the Go type name (`TypeName_ENUM_VALUE`), matching the `protoc-gen-go` convention. Map string values use bare proto names.
