...
- **Enum name maps**: Each enum gets `TypeName_name` (int32→string) and `TypeName_value` (string→int32) maps, plus a `String()` method. Enum constants are prefixed with the generated Go enum type name (e.g., `Color_COLOR_RED`, `Span_SpanKind_SPAN_KIND_SERVER`). Map string values use bare proto names.
