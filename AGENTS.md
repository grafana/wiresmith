...
- `(wiresmith.options.stdtime) = true` — swaps a singular `google.protobuf.Timestamp` field for a stdlib `time.Time`. Go zero `time.Time{}` is treated as "not set" (gogoproto-compatible); decoded times are normalised to UTC. On a proto3 `optional` field it instead produces `*time.Time` with real pointer-based presence (a `Has<Name>()` accessor, and an explicitly-set zero time round-trips as present) — the shape client_model-style protos need (e.g. `optional google.protobuf.Timestamp created_timestamp`). v1 scope: repeated, oneof, and pointer-option combinations are still rejected.
- `(wiresmith.options.stdduration) = true` — swaps a singular `google.protobuf.Duration` field for a stdlib `time.Duration`. `time.Duration(0)` is treated as "not set" (proto3 default-suppression); decode saturates to `math.{Max,Min}Int64` on overflow (proto Duration permits ~10000 years vs `time.Duration`'s ~292-year int64-nanos cap). Same `optional` → `*time.Duration` real-presence behavior as stdtime; same v1 rejection list otherwise.
- `(wiresmith.options.casttype) = "import/path.TypeName"` — renames the Go field type without changing wire encoding. Unlike customtype, the alias is a *defined type over the proto kind's natural Go shape* (e.g. `type UserID int64`); the marshal/unmarshal code bridges via Go casts. v1 scope: integer scalars, sint*, fixed*/sfixed*, bool, string, bytes. Rejects float/double (need bit-cast at every emit site), enum, message, map, oneof, repeated, optional.
- `(wiresmith.options.no_presence) = true` — **message-level** option (plus file-level `no_presence_all`; a per-message value overrides the file default): a **bitmap-only** concern — omits the `XXX_fieldsPresent` bitmap. Struct layout becomes declared-fields-only (gogo `nullable=false` parity — required for consumers doing unsafe casts between generated and domain types, e.g. Mimir), bitmap-backed `Has<F>()` is not emitted (proto3 `optional` keeps its nil-check Has), and present-but-empty nested messages do not survive a round-trip. It does **not** touch getter shape or storage: `Get<MsgField>()` returns `*T` (uniform getter shape; chained-call compatible) in every presence mode — deliberately diverging from gogo `nullable=false` value getters (safe per the 2026-06-18 consumer audit; pointer getters keep chained calls compiling). Implementation is a single switch: `fieldsForPresence` returns nil for opted-out messages; every bitmap consumer routes through it.
- `(wiresmith.options.enum_no_prefix) = true` — **enum-level** option (plus file-level `enum_no_prefix_all`; per-enum value overrides the file default): emits value constants without the `EnumName_` prefix (gogo `goproto_enum_prefix=false` parity). Identifiers only — name/value maps, String(), and wire format unchanged. Package-scope name collisions are the author's responsibility (caught by `go build`).

See [docs/extensions.md](docs/extensions.md) for the full rules and worked examples.

## Generated Compare() method

Every generated message receives a `Compare(other interface{}) int` method (-1/0/+1 like `bytes.Compare`, gogoproto-compatible nil/wrong-type preamble). Fields walk in ascending wire-tag order; floats compare bit-exact via `math.Float{32,64}bits`; maps sort their keys for a total order; oneof variants compare by declaration index then payload. The methods are emitted (alongside the per-message `Equal()`, co-located by wiresmith-2r4i) into a sibling `<name>_compare.pb.go` rather than the main `.pb.go` — emitting Compare next to the hot Marshal/Unmarshal in one file pushed the hot functions onto different cache sets and produced a measured +9% geomean regression on OTel hot benchmarks (UnmarshalMap +14%, MarshalSingleSpan +13%), the same icache-pressure failure mode the `_util.pb.go` split handles. See `compiler/generator/emit_compare.go` and the banner at the top of any `_compare.pb.go`.

## Generated Clone() method

Every generated message receives a `Clone() *T` deep-copy method (wiresmith-oz2l). nil receiver returns nil; otherwise it allocates a fresh message and deep-copies every field — bytes/slices/maps get fresh backing storage, pointer and nested-message fields recurse via their own `Clone()`, oneofs reconstruct the concrete variant, and the `XXX_fieldsPresent` bitmap is preserved, so `clone.Equal(orig)` holds. It is non-reflection (no `protoreflect`, no `proto.Clone`) and replaces wire-round-trip cloning. customtype/casttype fields are value-copied (casttype over `bytes` is deep-cloned) — see `compiler/generator/emit_clone.go`, `compiler/types/clone.go`, and the contract note in docs/extensions.md. Emitted into the cold `<name>_util.pb.go` (Clone is never on the hot path).
