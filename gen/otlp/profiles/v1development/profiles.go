// Code generated by grafana-protoc. DO NOT EDIT.
// source: opentelemetry/proto/profiles/v1development/profiles.proto

package profilesv1development

import (
	"encoding/binary"
	"fmt"
	"google.golang.org/protobuf/encoding/protowire"
	commonv1 "grafana-protoc/gen/otlp/common/v1"
	resourcev1 "grafana-protoc/gen/otlp/resource/v1"
	"grafana-protoc/gen/protohelpers"
)

type ProfilesDictionary struct {
	MappingTable   []Mapping
	LocationTable  []Location
	FunctionTable  []Function
	LinkTable      []Link
	StringTable    []string
	AttributeTable []KeyValueAndUnit
	StackTable     []Stack
}

type ProfilesData struct {
	ResourceProfiles []ResourceProfiles
	Dictionary       ProfilesDictionary
}

type ResourceProfiles struct {
	Resource      resourcev1.Resource
	ScopeProfiles []ScopeProfiles
	SchemaUrl     string
}

type ScopeProfiles struct {
	Scope     commonv1.InstrumentationScope
	Profiles  []Profile
	SchemaUrl string
}

type Profile struct {
	SampleType             ValueType
	Samples                []Sample
	TimeUnixNano           uint64
	DurationNano           uint64
	PeriodType             ValueType
	Period                 int64
	ProfileId              []byte
	DroppedAttributesCount uint32
	OriginalPayloadFormat  string
	OriginalPayload        []byte
	AttributeIndices       []int32
}

type Link struct {
	TraceId []byte
	SpanId  []byte
}

type ValueType struct {
	TypeStrindex int32
	UnitStrindex int32
}

type Sample struct {
	StackIndex         int32
	AttributeIndices   []int32
	LinkIndex          int32
	Values             []int64
	TimestampsUnixNano []uint64
}

type Mapping struct {
	MemoryStart      uint64
	MemoryLimit      uint64
	FileOffset       uint64
	FilenameStrindex int32
	AttributeIndices []int32
}

type Stack struct {
	LocationIndices []int32
}

type Location struct {
	MappingIndex     int32
	Address          uint64
	Lines            []Line
	AttributeIndices []int32
}

type Line struct {
	FunctionIndex int32
	Line          int64
	Column        int64
}

type Function struct {
	NameStrindex       int32
	SystemNameStrindex int32
	FilenameStrindex   int32
	StartLine          int64
}

type KeyValueAndUnit struct {
	KeyStrindex  int32
	Value        commonv1.AnyValue
	UnitStrindex int32
}

func (m *ProfilesDictionary) Size() int {
	var n int
	for i := range m.MappingTable {
		s := m.MappingTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	for i := range m.LocationTable {
		s := m.LocationTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	for i := range m.FunctionTable {
		s := m.FunctionTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	for i := range m.LinkTable {
		s := m.LinkTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	for _, v := range m.StringTable {
		n += 1 + protowire.SizeVarint(uint64(len(v))) + len(v)
	}
	for i := range m.AttributeTable {
		s := m.AttributeTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	for i := range m.StackTable {
		s := m.StackTable[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	return n
}

func (m *ProfilesData) Size() int {
	var n int
	for i := range m.ResourceProfiles {
		s := m.ResourceProfiles[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	{
		s := m.Dictionary.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	return n
}

func (m *ResourceProfiles) Size() int {
	var n int
	{
		s := m.Resource.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	for i := range m.ScopeProfiles {
		s := m.ScopeProfiles[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	if len(m.SchemaUrl) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.SchemaUrl))) + len(m.SchemaUrl)
	}
	return n
}

func (m *ScopeProfiles) Size() int {
	var n int
	{
		s := m.Scope.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	for i := range m.Profiles {
		s := m.Profiles[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	if len(m.SchemaUrl) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.SchemaUrl))) + len(m.SchemaUrl)
	}
	return n
}

func (m *Profile) Size() int {
	var n int
	{
		s := m.SampleType.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	for i := range m.Samples {
		s := m.Samples[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	if m.TimeUnixNano != 0 {
		n += 9
	}
	if m.DurationNano != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.DurationNano))
	}
	{
		s := m.PeriodType.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	if m.Period != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.Period))
	}
	if len(m.ProfileId) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.ProfileId))) + len(m.ProfileId)
	}
	if m.DroppedAttributesCount != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.DroppedAttributesCount))
	}
	if len(m.OriginalPayloadFormat) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.OriginalPayloadFormat))) + len(m.OriginalPayloadFormat)
	}
	if len(m.OriginalPayload) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.OriginalPayload))) + len(m.OriginalPayload)
	}
	if len(m.AttributeIndices) > 0 {
		var dataLen int
		for _, v := range m.AttributeIndices {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	return n
}

func (m *Link) Size() int {
	var n int
	if len(m.TraceId) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.TraceId))) + len(m.TraceId)
	}
	if len(m.SpanId) > 0 {
		n += 1 + protowire.SizeVarint(uint64(len(m.SpanId))) + len(m.SpanId)
	}
	return n
}

func (m *ValueType) Size() int {
	var n int
	if m.TypeStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.TypeStrindex))
	}
	if m.UnitStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.UnitStrindex))
	}
	return n
}

func (m *Sample) Size() int {
	var n int
	if m.StackIndex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.StackIndex))
	}
	if len(m.AttributeIndices) > 0 {
		var dataLen int
		for _, v := range m.AttributeIndices {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	if m.LinkIndex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.LinkIndex))
	}
	if len(m.Values) > 0 {
		var dataLen int
		for _, v := range m.Values {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	if len(m.TimestampsUnixNano) > 0 {
		dataLen := len(m.TimestampsUnixNano) * 8
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	return n
}

func (m *Mapping) Size() int {
	var n int
	if m.MemoryStart != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.MemoryStart))
	}
	if m.MemoryLimit != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.MemoryLimit))
	}
	if m.FileOffset != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.FileOffset))
	}
	if m.FilenameStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.FilenameStrindex))
	}
	if len(m.AttributeIndices) > 0 {
		var dataLen int
		for _, v := range m.AttributeIndices {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	return n
}

func (m *Stack) Size() int {
	var n int
	if len(m.LocationIndices) > 0 {
		var dataLen int
		for _, v := range m.LocationIndices {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	return n
}

func (m *Location) Size() int {
	var n int
	if m.MappingIndex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.MappingIndex))
	}
	if m.Address != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.Address))
	}
	for i := range m.Lines {
		s := m.Lines[i].Size()
		n += 1 + protowire.SizeVarint(uint64(s)) + s
	}
	if len(m.AttributeIndices) > 0 {
		var dataLen int
		for _, v := range m.AttributeIndices {
			dataLen += protowire.SizeVarint(uint64(v))
		}
		n += 1 + protowire.SizeVarint(uint64(dataLen)) + dataLen
	}
	return n
}

func (m *Line) Size() int {
	var n int
	if m.FunctionIndex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.FunctionIndex))
	}
	if m.Line != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.Line))
	}
	if m.Column != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.Column))
	}
	return n
}

func (m *Function) Size() int {
	var n int
	if m.NameStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.NameStrindex))
	}
	if m.SystemNameStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.SystemNameStrindex))
	}
	if m.FilenameStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.FilenameStrindex))
	}
	if m.StartLine != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.StartLine))
	}
	return n
}

func (m *KeyValueAndUnit) Size() int {
	var n int
	if m.KeyStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.KeyStrindex))
	}
	{
		s := m.Value.Size()
		if s > 0 {
			n += 1 + protowire.SizeVarint(uint64(s)) + s
		}
	}
	if m.UnitStrindex != 0 {
		n += 1 + protowire.SizeVarint(uint64(m.UnitStrindex))
	}
	return n
}

func (m *ProfilesDictionary) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *ProfilesDictionary) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	for iNdEx := len(m.StackTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.StackTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x3a
	}
	for iNdEx := len(m.AttributeTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.AttributeTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x32
	}
	for iNdEx := len(m.StringTable) - 1; iNdEx >= 0; iNdEx-- {
		i -= len(m.StringTable[iNdEx])
		copy(dAtA[i:], m.StringTable[iNdEx])
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.StringTable[iNdEx])))
		i--
		dAtA[i] = 0x2a
	}
	for iNdEx := len(m.LinkTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.LinkTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x22
	}
	for iNdEx := len(m.FunctionTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.FunctionTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	for iNdEx := len(m.LocationTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.LocationTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	for iNdEx := len(m.MappingTable) - 1; iNdEx >= 0; iNdEx-- {
		size := m.MappingTable[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x0a
	}
	return len(dAtA) - i
}

func (m *ProfilesData) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *ProfilesData) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	{
		size := m.Dictionary.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
		}
	}
	for iNdEx := len(m.ResourceProfiles) - 1; iNdEx >= 0; iNdEx-- {
		size := m.ResourceProfiles[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x0a
	}
	return len(dAtA) - i
}

func (m *ResourceProfiles) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *ResourceProfiles) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.SchemaUrl) > 0 {
		i -= len(m.SchemaUrl)
		copy(dAtA[i:], m.SchemaUrl)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SchemaUrl)))
		i--
		dAtA[i] = 0x1a
	}
	for iNdEx := len(m.ScopeProfiles) - 1; iNdEx >= 0; iNdEx-- {
		size := m.ScopeProfiles[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	{
		size := m.Resource.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x0a
		}
	}
	return len(dAtA) - i
}

func (m *ScopeProfiles) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *ScopeProfiles) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.SchemaUrl) > 0 {
		i -= len(m.SchemaUrl)
		copy(dAtA[i:], m.SchemaUrl)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SchemaUrl)))
		i--
		dAtA[i] = 0x1a
	}
	for iNdEx := len(m.Profiles) - 1; iNdEx >= 0; iNdEx-- {
		size := m.Profiles[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	{
		size := m.Scope.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x0a
		}
	}
	return len(dAtA) - i
}

func (m *Profile) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Profile) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.AttributeIndices) > 0 {
		var j int
		pStart := i
		for j = len(m.AttributeIndices) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.AttributeIndices[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x5a
	}
	if len(m.OriginalPayload) > 0 {
		i -= len(m.OriginalPayload)
		copy(dAtA[i:], m.OriginalPayload)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.OriginalPayload)))
		i--
		dAtA[i] = 0x52
	}
	if len(m.OriginalPayloadFormat) > 0 {
		i -= len(m.OriginalPayloadFormat)
		copy(dAtA[i:], m.OriginalPayloadFormat)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.OriginalPayloadFormat)))
		i--
		dAtA[i] = 0x4a
	}
	if m.DroppedAttributesCount != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.DroppedAttributesCount))
		i--
		dAtA[i] = 0x40
	}
	if len(m.ProfileId) > 0 {
		i -= len(m.ProfileId)
		copy(dAtA[i:], m.ProfileId)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.ProfileId)))
		i--
		dAtA[i] = 0x3a
	}
	if m.Period != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Period))
		i--
		dAtA[i] = 0x30
	}
	{
		size := m.PeriodType.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x2a
		}
	}
	if m.DurationNano != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.DurationNano))
		i--
		dAtA[i] = 0x20
	}
	if m.TimeUnixNano != 0 {
		i -= 8
		binary.LittleEndian.PutUint64(dAtA[i:], m.TimeUnixNano)
		i--
		dAtA[i] = 0x19
	}
	for iNdEx := len(m.Samples) - 1; iNdEx >= 0; iNdEx-- {
		size := m.Samples[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x12
	}
	{
		size := m.SampleType.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x0a
		}
	}
	return len(dAtA) - i
}

func (m *Link) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Link) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.SpanId) > 0 {
		i -= len(m.SpanId)
		copy(dAtA[i:], m.SpanId)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.SpanId)))
		i--
		dAtA[i] = 0x12
	}
	if len(m.TraceId) > 0 {
		i -= len(m.TraceId)
		copy(dAtA[i:], m.TraceId)
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.TraceId)))
		i--
		dAtA[i] = 0x0a
	}
	return len(dAtA) - i
}

func (m *ValueType) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *ValueType) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if m.UnitStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.UnitStrindex))
		i--
		dAtA[i] = 0x10
	}
	if m.TypeStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.TypeStrindex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *Sample) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Sample) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.TimestampsUnixNano) > 0 {
		for iNdEx := len(m.TimestampsUnixNano) - 1; iNdEx >= 0; iNdEx-- {
			i -= 8
			binary.LittleEndian.PutUint64(dAtA[i:], m.TimestampsUnixNano[iNdEx])
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(len(m.TimestampsUnixNano)*8))
		i--
		dAtA[i] = 0x2a
	}
	if len(m.Values) > 0 {
		var j int
		pStart := i
		for j = len(m.Values) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Values[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x22
	}
	if m.LinkIndex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LinkIndex))
		i--
		dAtA[i] = 0x18
	}
	if len(m.AttributeIndices) > 0 {
		var j int
		pStart := i
		for j = len(m.AttributeIndices) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.AttributeIndices[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x12
	}
	if m.StackIndex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.StackIndex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *Mapping) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Mapping) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.AttributeIndices) > 0 {
		var j int
		pStart := i
		for j = len(m.AttributeIndices) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.AttributeIndices[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x2a
	}
	if m.FilenameStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.FilenameStrindex))
		i--
		dAtA[i] = 0x20
	}
	if m.FileOffset != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.FileOffset))
		i--
		dAtA[i] = 0x18
	}
	if m.MemoryLimit != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.MemoryLimit))
		i--
		dAtA[i] = 0x10
	}
	if m.MemoryStart != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.MemoryStart))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *Stack) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Stack) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.LocationIndices) > 0 {
		var j int
		pStart := i
		for j = len(m.LocationIndices) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.LocationIndices[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x0a
	}
	return len(dAtA) - i
}

func (m *Location) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Location) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if len(m.AttributeIndices) > 0 {
		var j int
		pStart := i
		for j = len(m.AttributeIndices) - 1; j >= 0; j-- {
			i = protohelpers.EncodeVarint(dAtA, i, uint64(m.AttributeIndices[j]))
		}
		i = protohelpers.EncodeVarint(dAtA, i, uint64(pStart-i))
		i--
		dAtA[i] = 0x22
	}
	for iNdEx := len(m.Lines) - 1; iNdEx >= 0; iNdEx-- {
		size := m.Lines[iNdEx].MarshalToSizedBuffer(dAtA[:i])
		i -= size
		i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
		i--
		dAtA[i] = 0x1a
	}
	if m.Address != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Address))
		i--
		dAtA[i] = 0x10
	}
	if m.MappingIndex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.MappingIndex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *Line) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Line) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if m.Column != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Column))
		i--
		dAtA[i] = 0x18
	}
	if m.Line != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.Line))
		i--
		dAtA[i] = 0x10
	}
	if m.FunctionIndex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.FunctionIndex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *Function) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *Function) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if m.StartLine != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.StartLine))
		i--
		dAtA[i] = 0x20
	}
	if m.FilenameStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.FilenameStrindex))
		i--
		dAtA[i] = 0x18
	}
	if m.SystemNameStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.SystemNameStrindex))
		i--
		dAtA[i] = 0x10
	}
	if m.NameStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.NameStrindex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func (m *KeyValueAndUnit) Marshal() ([]byte, error) {
	size := m.Size()
	if size == 0 {
		return nil, nil
	}
	dAtA := make([]byte, size)
	n := m.MarshalToSizedBuffer(dAtA)
	return dAtA[:n], nil
}

func (m *KeyValueAndUnit) MarshalToSizedBuffer(dAtA []byte) int {
	i := len(dAtA)
	if m.UnitStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.UnitStrindex))
		i--
		dAtA[i] = 0x18
	}
	{
		size := m.Value.MarshalToSizedBuffer(dAtA[:i])
		if size > 0 {
			i -= size
			i = protohelpers.EncodeVarint(dAtA, i, uint64(size))
			i--
			dAtA[i] = 0x12
		}
	}
	if m.KeyStrindex != 0 {
		i = protohelpers.EncodeVarint(dAtA, i, uint64(m.KeyStrindex))
		i--
		dAtA[i] = 0x08
	}
	return len(dAtA) - i
}

func skipField(b []byte, num protowire.Number, typ protowire.Type) (int, error) {
	switch typ {
	case protowire.VarintType:
		_, n := protowire.ConsumeVarint(b)
		if n < 0 {
			return 0, fmt.Errorf("invalid varint")
		}
		return n, nil
	case protowire.Fixed32Type:
		if len(b) < 4 {
			return 0, fmt.Errorf("truncated fixed32")
		}
		return 4, nil
	case protowire.Fixed64Type:
		if len(b) < 8 {
			return 0, fmt.Errorf("truncated fixed64")
		}
		return 8, nil
	case protowire.BytesType:
		_, n := protowire.ConsumeBytes(b)
		if n < 0 {
			return 0, fmt.Errorf("invalid bytes")
		}
		return n, nil
	case protowire.StartGroupType:
		_, n := protowire.ConsumeGroup(num, b)
		if n < 0 {
			return 0, fmt.Errorf("invalid group")
		}
		return n, nil
	default:
		return 0, fmt.Errorf("unknown wire type %d", typ)
	}
}

func (m *ProfilesDictionary) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field1count int
		var field2count int
		var field3count int
		var field4count int
		var field5count int
		var field6count int
		var field7count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 1:
				field1count++
			case 2:
				field2count++
			case 3:
				field3count++
			case 4:
				field4count++
			case 5:
				field5count++
			case 6:
				field6count++
			case 7:
				field7count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field1count > 0 {
			m.MappingTable = make([]Mapping, 0, field1count)
		}
		if field2count > 0 {
			m.LocationTable = make([]Location, 0, field2count)
		}
		if field3count > 0 {
			m.FunctionTable = make([]Function, 0, field3count)
		}
		if field4count > 0 {
			m.LinkTable = make([]Link, 0, field4count)
		}
		if field5count > 0 {
			m.StringTable = make([]string, 0, field5count)
		}
		if field6count > 0 {
			m.AttributeTable = make([]KeyValueAndUnit, 0, field6count)
		}
		if field7count > 0 {
			m.StackTable = make([]Stack, 0, field7count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // mapping_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.MappingTable = append(m.MappingTable, Mapping{})
			if err := m.MappingTable[len(m.MappingTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 2: // location_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.LocationTable = append(m.LocationTable, Location{})
			if err := m.LocationTable[len(m.LocationTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 3: // function_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.FunctionTable = append(m.FunctionTable, Function{})
			if err := m.FunctionTable[len(m.FunctionTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 4: // link_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.LinkTable = append(m.LinkTable, Link{})
			if err := m.LinkTable[len(m.LinkTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 5: // string_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeString(b)
			if n < 0 {
				return fmt.Errorf("invalid string")
			}
			m.StringTable = append(m.StringTable, v)
			b = b[n:]
		case 6: // attribute_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.AttributeTable = append(m.AttributeTable, KeyValueAndUnit{})
			if err := m.AttributeTable[len(m.AttributeTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 7: // stack_table
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.StackTable = append(m.StackTable, Stack{})
			if err := m.StackTable[len(m.StackTable)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *ProfilesData) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field1count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 1:
				field1count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field1count > 0 {
			m.ResourceProfiles = make([]ResourceProfiles, 0, field1count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // resource_profiles
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.ResourceProfiles = append(m.ResourceProfiles, ResourceProfiles{})
			if err := m.ResourceProfiles[len(m.ResourceProfiles)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 2: // dictionary
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.Dictionary.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *ResourceProfiles) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field2count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 2:
				field2count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field2count > 0 {
			m.ScopeProfiles = make([]ScopeProfiles, 0, field2count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // resource
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.Resource.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 2: // scope_profiles
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.ScopeProfiles = append(m.ScopeProfiles, ScopeProfiles{})
			if err := m.ScopeProfiles[len(m.ScopeProfiles)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 3: // schema_url
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeString(b)
			if n < 0 {
				return fmt.Errorf("invalid string")
			}
			m.SchemaUrl = v
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *ScopeProfiles) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field2count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 2:
				field2count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field2count > 0 {
			m.Profiles = make([]Profile, 0, field2count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // scope
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.Scope.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 2: // profiles
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.Profiles = append(m.Profiles, Profile{})
			if err := m.Profiles[len(m.Profiles)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 3: // schema_url
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeString(b)
			if n < 0 {
				return fmt.Errorf("invalid string")
			}
			m.SchemaUrl = v
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Profile) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field2count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 2:
				field2count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field2count > 0 {
			m.Samples = make([]Sample, 0, field2count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // sample_type
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.SampleType.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 2: // samples
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.Samples = append(m.Samples, Sample{})
			if err := m.Samples[len(m.Samples)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 3: // time_unix_nano
			if typ != protowire.Fixed64Type {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeFixed64(b)
			if n < 0 {
				return fmt.Errorf("invalid fixed64")
			}
			m.TimeUnixNano = v
			b = b[n:]
		case 4: // duration_nano
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.DurationNano = v
			b = b[n:]
		case 5: // period_type
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.PeriodType.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 6: // period
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.Period = int64(v)
			b = b[n:]
		case 7: // profile_id
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.ProfileId = append(m.ProfileId[:0], v...)
			b = b[n:]
		case 8: // dropped_attributes_count
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.DroppedAttributesCount = uint32(v)
			b = b[n:]
		case 9: // original_payload_format
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeString(b)
			if n < 0 {
				return fmt.Errorf("invalid string")
			}
			m.OriginalPayloadFormat = v
			b = b[n:]
		case 10: // original_payload
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.OriginalPayload = append(m.OriginalPayload[:0], v...)
			b = b[n:]
		case 11: // attribute_indices
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.AttributeIndices) == 0 {
					m.AttributeIndices = make([]int32, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.AttributeIndices = append(m.AttributeIndices, int32(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.AttributeIndices = append(m.AttributeIndices, int32(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Link) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // trace_id
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.TraceId = append(m.TraceId[:0], v...)
			b = b[n:]
		case 2: // span_id
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.SpanId = append(m.SpanId[:0], v...)
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *ValueType) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // type_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.TypeStrindex = int32(v)
			b = b[n:]
		case 2: // unit_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.UnitStrindex = int32(v)
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Sample) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // stack_index
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.StackIndex = int32(v)
			b = b[n:]
		case 2: // attribute_indices
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.AttributeIndices) == 0 {
					m.AttributeIndices = make([]int32, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.AttributeIndices = append(m.AttributeIndices, int32(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.AttributeIndices = append(m.AttributeIndices, int32(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		case 3: // link_index
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.LinkIndex = int32(v)
			b = b[n:]
		case 4: // values
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.Values) == 0 {
					m.Values = make([]int64, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.Values = append(m.Values, int64(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.Values = append(m.Values, int64(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		case 5: // timestamps_unix_nano
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				if elementCount := len(data) / 8; elementCount != 0 && len(m.TimestampsUnixNano) == 0 {
					m.TimestampsUnixNano = make([]uint64, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeFixed64(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed fixed64")
					}
					m.TimestampsUnixNano = append(m.TimestampsUnixNano, v)
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.Fixed64Type {
				v, n := protowire.ConsumeFixed64(b)
				if n < 0 {
					return fmt.Errorf("invalid fixed64")
				}
				m.TimestampsUnixNano = append(m.TimestampsUnixNano, v)
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Mapping) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // memory_start
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.MemoryStart = v
			b = b[n:]
		case 2: // memory_limit
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.MemoryLimit = v
			b = b[n:]
		case 3: // file_offset
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.FileOffset = v
			b = b[n:]
		case 4: // filename_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.FilenameStrindex = int32(v)
			b = b[n:]
		case 5: // attribute_indices
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.AttributeIndices) == 0 {
					m.AttributeIndices = make([]int32, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.AttributeIndices = append(m.AttributeIndices, int32(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.AttributeIndices = append(m.AttributeIndices, int32(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Stack) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // location_indices
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.LocationIndices) == 0 {
					m.LocationIndices = make([]int32, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.LocationIndices = append(m.LocationIndices, int32(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.LocationIndices = append(m.LocationIndices, int32(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Location) Unmarshal(b []byte) error {
	if len(b) >= 256 {
		tmp := b
		var field3count int
		for len(tmp) > 0 {
			num, typ, tagLen := protowire.ConsumeTag(tmp)
			if tagLen < 0 {
				break
			}
			tmp = tmp[tagLen:]
			switch num {
			case 3:
				field3count++
			}
			var skip int
			switch typ {
			case protowire.VarintType:
				_, skip = protowire.ConsumeVarint(tmp)
			case protowire.Fixed32Type:
				skip = 4
			case protowire.Fixed64Type:
				skip = 8
			case protowire.BytesType:
				_, skip = protowire.ConsumeBytes(tmp)
			case protowire.StartGroupType:
				_, skip = protowire.ConsumeGroup(num, tmp)
			}
			if skip < 0 || skip > len(tmp) {
				break
			}
			tmp = tmp[skip:]
		}
		if field3count > 0 {
			m.Lines = make([]Line, 0, field3count)
		}
	}
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // mapping_index
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.MappingIndex = int32(v)
			b = b[n:]
		case 2: // address
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.Address = v
			b = b[n:]
		case 3: // lines
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			m.Lines = append(m.Lines, Line{})
			if err := m.Lines[len(m.Lines)-1].Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 4: // attribute_indices
			if typ == protowire.BytesType {
				data, n := protowire.ConsumeBytes(b)
				if n < 0 {
					return fmt.Errorf("invalid packed field")
				}
				var elementCount int
				for _, b := range data {
					if b < 128 {
						elementCount++
					}
				}
				if elementCount != 0 && len(m.AttributeIndices) == 0 {
					m.AttributeIndices = make([]int32, 0, elementCount)
				}
				for len(data) > 0 {
					v, vn := protowire.ConsumeVarint(data)
					if vn < 0 {
						return fmt.Errorf("invalid packed varint")
					}
					m.AttributeIndices = append(m.AttributeIndices, int32(v))
					data = data[vn:]
				}
				b = b[n:]
			} else if typ == protowire.VarintType {
				v, n := protowire.ConsumeVarint(b)
				if n < 0 {
					return fmt.Errorf("invalid varint")
				}
				m.AttributeIndices = append(m.AttributeIndices, int32(v))
				b = b[n:]
			} else {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
			}
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Line) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // function_index
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.FunctionIndex = int32(v)
			b = b[n:]
		case 2: // line
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.Line = int64(v)
			b = b[n:]
		case 3: // column
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.Column = int64(v)
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *Function) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // name_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.NameStrindex = int32(v)
			b = b[n:]
		case 2: // system_name_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.SystemNameStrindex = int32(v)
			b = b[n:]
		case 3: // filename_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.FilenameStrindex = int32(v)
			b = b[n:]
		case 4: // start_line
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.StartLine = int64(v)
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}

func (m *KeyValueAndUnit) Unmarshal(b []byte) error {
	for len(b) > 0 {
		num, typ, tagLen := protowire.ConsumeTag(b)
		if tagLen < 0 {
			return fmt.Errorf("invalid tag")
		}
		b = b[tagLen:]
		switch num {
		case 1: // key_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.KeyStrindex = int32(v)
			b = b[n:]
		case 2: // value
			if typ != protowire.BytesType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeBytes(b)
			if n < 0 {
				return fmt.Errorf("invalid bytes")
			}
			if err := m.Value.Unmarshal(v); err != nil {
				return err
			}
			b = b[n:]
		case 3: // unit_strindex
			if typ != protowire.VarintType {
				n, err := skipField(b, num, typ)
				if err != nil {
					return err
				}
				b = b[n:]
				continue
			}
			v, n := protowire.ConsumeVarint(b)
			if n < 0 {
				return fmt.Errorf("invalid varint")
			}
			m.UnitStrindex = int32(v)
			b = b[n:]
		default:
			n, err := skipField(b, num, typ)
			if err != nil {
				return err
			}
			b = b[n:]
		}
	}
	return nil
}
