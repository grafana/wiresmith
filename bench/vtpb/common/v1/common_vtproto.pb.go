...
func (m *AnyValue_StringValueStrindex) MarshalToVT(dAtA []byte) (int, error) {
	size := m.SizeVT()
	return m.MarshalToSizedBufferVT(dAtA[:size])
}

func (m *AnyValue_StringValueStrindex) MarshalToSizedBufferVT(dAtA []byte) (int, error) {
	i := len(dAtA)
	i = protohelpers.EncodeVarint(dAtA, i, uint64(m.StringValueStrindex))
	i--
	dAtA[i] = 0x40
	return len(dAtA) - i, nil
}
...
func (m *AnyValue_StringValueStrindex) SizeVT() (n int) {
	if m == nil {
		return 0
	}
	var l int
	_ = l
	n += 1 + protohelpers.SizeOfVarint(uint64(m.StringValueStrindex))
	return n
}
...
func (m *AnyValue) UnmarshalVT(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 8:
			...
			m.Value = &AnyValue_StringValueStrindex{StringValueStrindex: v}
		default:
			iNdEx = preIndex
