...
- If a regenerated file needs irreducible hand-edits, store them as a re-appliable patch (`X.pb.go.expdiff`, applied by script after every regen — see mimir's `tools/apply-expected-diffs.sh`), never as silent manual edits. Report the expdiff line count in the migration log and drive it toward zero as upstream fixes land (mimir: 2144 → 1103 → shrinking; e.g. as of `8c63b47` a file of only field-less messages no longer emits an unused `protowire` import — retire that expdiff after re-pinning).
...
- **Registration: wiresmith now emits OFFICIAL-runtime registration** (`protoimpl.TypeBuilder`) into the cold `<name>_util.pb.go` sibling by default — so `google.golang.org/protobuf` `protoregistry` resolution works out of the box. When auditing registration, grep `*_util.pb.go`, NOT the main `.pb.go` (which carries none — a first-pass grep of the main file finds zero and misleads). **The gogo registry is a separate world**: wiresmith types stay invisible to gogo `types.MarshalAny` / `proto.MessageType` / gogo-`jsonpb` / TypeUrl (DB-19), which fail silently. A manual gogo `proto.RegisterType` `init()` shim is still the fix *where a gogo reflection/jsonpb/Any path actually survives* — but confirm one survives first: Prometheus needed ZERO gogo shims (no jsonpb/Any/MarshalAny/unsafe on prompb), and the official-runtime registration didn't collide (`go test ./prompb/...` clean). Grep for `MarshalAny|MessageType|jsonpb` before declaring a cluster done. **Also check for an OFFICIAL-registry collision:** if an external official-runtime module in the same binary registers the SAME proto package (import path / message full name) into `protoregistry.GlobalFiles`/`GlobalTypes`, wiresmith's default global registration panics at init (precedent: Prometheus generating `io.prometheus.client` with wiresmith while `github.com/prometheus/client_model/go` registers the same package officially; gogo never collided — separate registry). Remedy: file-level `option (wiresmith.options.no_registration) = true;` (field 50044, ≥ databases `8c63b47`) — the file registers into a package-LOCAL registry, so `ProtoReflect`/`String`/`Clone`/`proto.Marshal` keep working and the global registries stay untouched (caveat: `FileDescriptor.Imports()` yields placeholders); see docs/extensions.md.
- **gogo text-format fixtures break silently.** gogo `proto.UnmarshalText` resolves ENUM values through the gogo GLOBAL registry, so migrating a type off gogo removes its `RegisterEnum` and any text fixture referencing its enums fails (`invalid value for …MetricType`). Grep consumers for `proto.UnmarshalText` / `RegisterEnum` coupling BEFORE migrating a type. Remedies: parse the fixture with the official `prototext.Unmarshal` against an official-registered wire-identical type, or convert the fixture to a Go struct literal.
- **A pointer-drop breaks range-loop mutation silently.** Converting `[]*T` → `[]T` (or `*T` → `T`) turns `for _, x := range s { x.Field = … }` into a mutation of the range COPY, not the original element — no compile error, no vet warning, and the code still reads as correct. Two real instances from Tempo's OTLP pointer-drop, both filtering/deduping `ScopeSpans.Spans`: `modules/blockbuilder/live_traces_iter.go` (span dedup) and `modules/generator/instance.go`'s `preprocessSpans` (ingestion-slack filter) — both caught only by tests asserting downstream counts (a unit test and an e2e), not by inspection. Fix: index-based iteration (`for i := range s { x := &s[i]; … }`). After ANY pointer-drop, sweep every range loop and value copy of the affected types for element mutation (field assignment, or a pointer-receiver method called on the loop variable) BEFORE trusting a green build; prefer tests asserting downstream counts/contents — that's what caught both.
