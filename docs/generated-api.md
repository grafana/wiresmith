...
| `String() string`                                   | Hand-rolled **proto-text** debug dump (`field: value nested: {…}`), the same shape protoc-gen-go/gogo emit: proto field names, enums by value name, quoted strings/bytes, sorted map entries, zero/absent fields omitted. Content- and byte-deterministic (pointers dereferenced — no heap addresses; no detrand jitter). Not `MessageStringOf` (that needs field reflection, which panics here). Emitted into the companion `<name>_util.pb.go`. |
...
| `Clone() *T`                                        | Non-reflection deep copy. `nil` receiver returns `nil`; otherwise allocates a fresh message and deep-copies every field — bytes/slices/maps get fresh backing storage, pointer and nested-message fields recurse through their own `Clone()`, oneofs reconstruct the concrete variant, and the presence bitmap is preserved, so `clone.Equal(orig)` holds. customtype/casttype fields are copied by value (see [extensions.md](extensions.md)). Replaces wire-round-trip or reflection-based cloning. Emitted into the companion `<name>_util.pb.go`. |
