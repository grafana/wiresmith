...
- **No proto2 or extensions** (other than the wiresmith options) — out of scope for the OpenTelemetry-style workloads wiresmith targets. gRPC `service` definitions **are** supported: a file declaring `service` blocks emits `<name>_grpc.pb.go` stubs via a vendored `protoc-gen-go-grpc` (see the "protoc / buf plugin" section of [cli.md](cli.md)).
