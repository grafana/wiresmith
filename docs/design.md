...
- **No proto2 or extensions** (other than the wiresmith options) — out of scope for the observability-pipeline workloads wiresmith targets. (gRPC *services* **are** supported: a `.proto` declaring `service` blocks emits `<name>_grpc.pb.go` stubs via a vendored `protoc-gen-go-grpc` — see [buf.md](buf.md).)
- **Well-known types are supported only where consumers need them:** `Timestamp` / `Duration` via the `stdtime` / `stdduration` options (singular or proto3-`optional`), and `google.protobuf.Any` via a shipped replacement package (`types/known/anypb`) that references resolve to automatically (see `compiler/generator/wellknown.go`). The replacement omits proto-registry registration — the official runtime already registers `google.protobuf.Any` — and hand-writes a `ProtoReflect` that delegates to the official descriptor. Because the shipped helpers resolve type URLs through the official `protoregistry.GlobalTypes` only, consumers whose `Any` payloads are registered in the gogo registry keep a customtype bridge; see the `google.protobuf.Any` section of [extensions.md](extensions.md#googleprotobufany). Other WKTs (Empty, Struct, FieldMask, wrappers, Api/Type/SourceContext) are out of scope. Referencing one of these unsupported WKTs in a message field — or referencing Timestamp/Duration without the matching stdtime/stdduration option — is rejected at generation time (`compiler/generator/wellknown_validate.go`) rather than silently producing Go code that fails to compile: without that check, the field would resolve to the official `google.golang.org/protobuf` type while wiresmith still emitted its own wire-method calls on it, and the mismatch only surfaced at `go build`.
