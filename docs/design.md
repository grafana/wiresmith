...
- **Pre-scan pre-allocation.** A lightweight pre-scan during unmarshal counts repeated elements before allocating slices at exact capacity. With value-type fields, exact capacity matters more than for pointer-based libraries — a wasted slot in `[]Span` costs `sizeof(Span)` rather than 8 bytes.

- **Prealloc reuses caller capacity (pooling contract).** When the pre-scan finds a repeated slice whose existing `cap` already fits the counted elements, it truncates to `[:0]` and reuses the backing array instead of allocating — the pooled-message pattern (a message reset and reused from a `sync.Pool` keeps its buffers across decodes). The flip side is the documented contract: `Unmarshal` expects a `Reset()` (or fresh) message; re-unmarshaling into a used message while holding references to slices from the previous decode will observe them being overwritten. This was already true for payloads below the pre-scan threshold (the plain decode path appends into existing capacity); the reuse makes the behavior uniform instead of dependent on payload size. Stale elements never leak into decoded values — the decode loop appends a zero value into each slot before unmarshaling into it.
